...
def test_s3_file_delete(
    client_s3_storage_1: object, s3_client: object, s3_resource: object, s3_bucket: str
) -> None:
    """Testing DELETE resource

    Args:
        client_s3_storage_1 (fixture): The test client.
        s3_client (fixture): A S3 client object.
        s3_resource (fixture): A S3 resource object.
        s3_bucket (fixture): The s3 bucket name.
    """
    key = f'{uuid4()}'
    contents: object = io.BytesIO(key.encode())
    ...
    response: Result = client_s3_storage_1.simulate_delete('/middleware', params=params)
...
def test_s3_file_delete_404(client_s3_storage_1: object) -> None:
    """Testing DELETE resource
...
def test_s3_file_exists(
    ...
    params = {'filename': f'{key}.txt'}
    response: Result = client_s3_storage_1.simulate_get('/middleware', params=params)
    assert response.status_code == 200
    assert response.text == key
    s3_resource.Object(s3_bucket, f'{key}.txt').delete()


def test_s3_does_not_exists(client_s3_storage_1: object) -> None:
    """Testing GET resource
...
def test_s3_file_upload(
    ...
    response: Result = client_s3_storage_1.simulate_post('/middleware', body=data, headers=headers)
    assert response.status_code == 200
...
def test_s3_file_upload_fail(client_s3_storage_2: object) -> None:
    """Testing POST resource
